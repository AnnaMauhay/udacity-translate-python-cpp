...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	for (int row = 0; row < grid.size(); row++) {
      ...
      for (int j = 0; j < grid[0].size(); j++) {
			tempRow.push_back(grid[i][j] / denominator);
		}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
  ...
  float center_prob, corner_prob, adjacent_prob, grid_val;
  ...
  for (int i = 0; i < 3; i++){
    ...
    for (int j = 0; j < 3; j++){
      if ((i==0 || i==2) && (j==0 || j==2)){
        tempRow.push_back(corner_prob);
      } else if (i==1 && j==1){
        tempRow.push_back(center_prob);
      } else tempRow.push_back(adjacent_prob);
    }
    window.push_back(tempRow);
  }
    ...
    for (int j=0; j<width; j++){
      ...
      for (int dx = -1; dx < 2; dx++){
         for (int dy = -1; dy < 2; dy++){
           ...
           new_i = (i + dy) % height;
           new_j = (j + dx) % width;
           if ((new_i >= 0) && (new_j >= 0) ) newGrid[new_i][new_j] += (mult * grid_val);
         }
      }
    }
  }
